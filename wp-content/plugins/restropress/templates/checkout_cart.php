...
<table id="rpress_checkout_cart " class="rpress-cart ajaxed">
  ...
  <tbody>
    ...
    <tr>
      <th colspan="3" class="rpress_get_subtotal sss">
  ...
  <tfoot>
    ...
    <tr class="rpress_cart_footer_rowsss">
      <?php do_action( 'rpress_checkout_table_footer_first' ); ?>
	  
      <th colspan="<?php echo rpress_checkout_cart_columns(); ?>" class="rpress_cart_total">
	  <?php //$tt =  rpress_get_cart_subtotal();
	//$pricenocurrency  = preg_replace( '/[^.\d]/', '', $tt  );
	//echo $pricenocurrency;
		$cstval = rpress_cart_tip();
		//echo $cstval;
		//$final = $pricenocurrency+$cstval;
		
		//$d = RPRESS()->cart->total( $final );
		//echo $d;
	  ?>
	  <?php esc_html_e( 'Total', 'restropress' ); ?>: <span class="rpress_cart_amount pull-right" data-subtotal="<?php echo rpress_get_cart_subtotal(); ?>" data-total="<?php echo rpress_get_cart_total(); ?>"><?php rpress_cart_total(); ?></span>
