...
<?php 
$ttl =  rpress_currency_filter( rpress_format_amount( rpress_get_cart_total() ) ); 
?>		
<?php esc_html_e( 'Total (', 'restropress' ); ?><span class="rpress-cart-quantity" <?php echo wp_kses_post( $display ); ?> ><?php echo esc_html( $cart_quantity ); ?></span><?php esc_html_e( ' Items)', 'restropress' ); ?><span class="cart-total"><?php echo rpress_currency_filter( rpress_format_amount( rpress_get_cart_total() ) ); ?></span></li>
...
<input type="hidden" name="tipvalue" class="giventip" value="15">
<script>
	jQuery('input:radio[name="tipchoose"]').change(function(){
		var tip_val = jQuery("input[type='radio'][name='tipchoose']:checked").val();
        if (jQuery(this).is(':checked')) {
            jQuery('.giventip').val(tip_val);
        }
    });
</script>	